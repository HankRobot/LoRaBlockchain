...
class Signature {
    ...
    verify(message, public_keys) {
        ...
        challenge = new bn_js_1.default(challenge, 16).toString('hex');
        return challenge === c_sum;
    }
    c_summation(c_array, hasher) {
        let summation = new bn_js_1.default(0, 16);
    ...
    generate_ll(public_keys, c_array, r_array, hasher) {
        ...
        for (let i = 0; i < public_keys.length; i++) {
            let rG = hasher.G.mul(new bn_js_1.default(r_array[i], 16));
            let cP = public_keys[i].point.mul(new bn_js_1.default(c_array[i], 16));
    ...
    generate_rr(public_keys, c_array, r_array, hasher, key_image) {
        let rr_array = [];
        for (let i = 0; i < public_keys.length; i++) {
            let cI = key_image.mul(new bn_js_1.default(c_array[i], 16));
            let HpP = hasher.hash_point(public_keys[i].point);
            let rHp = HpP.mul(new bn_js_1.default(r_array[i], 16));
