...
class PrivateKey {
    constructor(value, hasher) {
        ...
        this.hasher = hasher;
        this.public_key = new public_key_js_1.default(this.hasher.G.mul(this.value), this.hasher);
    ...
    sign(message, foreign_keys) {
        ...
        return new signature_js_1.default(this.key_image, c_array, r_array, public_keys, this.hasher);
    }
    generate_r(all_keys, q_array, w_array, c_array, challenge) {
        let r_array = [];
        for (let i = 0; i < all_keys.length; i++) {
            if (all_keys[i] instanceof public_key_js_1.default) {
