...
class PrivateKey {
    constructor(value, hasher) {
        ...
        this.hasher = hasher;
        this.public_key = new public_key_1.PublicKey(this.hasher.G.mul(this.value), this.hasher);
    ...
    sign(message, foreign_keys) {
        ...
        return new signature_1.Signature(this.key_image, c_array, r_array, public_keys, this.hasher);
    }
    generate_r(all_keys, q_array, w_array, c_array, challenge) {
        let r_array = [];
        for (let i = 0; i < all_keys.length; i++) {
            if (all_keys[i] instanceof public_key_1.PublicKey) {
